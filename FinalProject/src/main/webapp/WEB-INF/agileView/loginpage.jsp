...
$(document).ready(function(){
	...
	$('#login').on('click',function(){
		...
		if(joinChk!=0){
			$('#email-error').show();
			$('#email-error').text('등록되지 않는 이메일 주소 입니다.');
			$('#email').val('');
			return false;
		}else{
			$('#email-error').hide();
	...
	<div class="section-wrapper layout-grid-frame">
		<div id="signup-form" class="layout-twothirds-center">
			...
			<div id="signup-password">
				<h1>Welcome to Love your project </h1>
				<label for="email">Email</label> 
				<div id="email-error" class="hidden error-message"></div>
				
				<input type="email" name="email"
					id="email" tabindex="0" placeholder="ex) abcd@abcd.abc">
				...
				<div id="pass-error" class="hidden error-message">
					...
					name="password" id="password" tabindex="0" placeholder="비밀번호">
					 ...
					 <input id="login"
					tabindex="0" type="button" class="button button-green" value="로그인">
					<input id="join"
